...
def task_detail(request: Request, task_id: str) -> TaskDetailResponse:
    ...
    if task.assigned_agent:
        ...
        if log_path is not None:
            content, size = read_log_tail(log_path, tail_bytes=4096)
            log_tail = content
            log_size = size
    ...
    if hasattr(task, "progress_log") and task.progress_log:
        for entry in task.progress_log:
            progress_entries.append(
                {
                    "message": entry.get("message", "") if isinstance(entry, dict) else getattr(entry, "message", ""),
                    "percent": entry.get("percent", 0) if isinstance(entry, dict) else getattr(entry, "percent", 0),
                    "timestamp": entry.get("timestamp", 0)
                    if isinstance(entry, dict)
                    else getattr(entry, "timestamp", 0),
                }
            )
