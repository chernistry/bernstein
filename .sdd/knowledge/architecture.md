...
- `bootstrap` — Bootstrap: parse seed -> init .sdd -> start server -> plan -> orchestrate. [BootstrapResult, preflight_checks, ensure_sdd, create_router, bootstrap_from_seed, run_watchdog]
...
- `cost_history` — Cost history persistence and alert logic. [DailyCostSnapshot, CostTrend, CostAlert, append_daily_snapshot, upsert_daily_snapshot, load_history]
...
- `costs` — Cost budget routes. [get_costs, get_cost_live, get_cost_alerts, get_cost_history, get_cost_budget]
- `dashboard` — Dashboard-specific routes — file lock inspection endpoint. [file_locks_endpoint]
- `status` — Status, health, metrics, dashboard, and SSE event routes. [status_dashboard, health_check, metrics_endpoint, dashboard_page, dashboard_data, build_alerts]
...
- `webhooks` — GitHub webhook route and alerts endpoint. [get_alerts, github_webhook]
...
- `judge` — LLM judge — evaluate code quality of agent-produced changes. [CircuitBreakerTripped, JudgeVerdict, EvalJudge, judge_code_change]
...
- `governance` — Adaptive governance for the evolution system. [EvolutionWeights, ProjectContext, GovernanceEntry, AdaptiveGovernor, log_evolution_decision]
...
- `test_agent_discovery` — Tests for bernstein.core.agent_discovery — agent auto-discovery. [TestExtractVersion, TestDetectCodex, TestDetectGemini, TestDetectClaude, TestDetectQwen, TestDetectAider]
...
- `test_claim_role_matching` — Tests for role-locked task claiming in the orchestrator. [test_claim_by_id_with_matching_role_succeeds, test_claim_by_id_with_mismatched_role_fails, test_claim_batch_with_matching_roles_succeeds, test_claim_batch_with_mixed_roles_fails_for_mismatched, test_claim_batch_rejects_all_mismatched]
...
- `test_orchestrator` — Tests for the Orchestrator — httpx calls and spawner are always mocked. [TestTaskFromDict, TestGroupByRole, TestPrioritizeStarvingRoles, TestTickStarvingRolePriority, TestPerRoleCapDistribution, TestRoleFilteredClaiming]
...
- `test_project_memory` — Tests for cross-run project memory. [TestAppendToProjectMemory, TestGetRecentProjectMemory, TestGatherProjectMemory, TestProjectMemoryInContext, temp_sdd_dir]
...
- `test_zero_config` — Tests for zero-config agent setup — auto-detection on first run. [TestSeedConfigDefault, TestPreflightChecksAutoMode, TestAutoWriteBernsteinYaml, TestBootstrapFromGoalDefaults, TestGenerateDefaultYaml, TestCLIOverrides]
