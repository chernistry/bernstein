...
from bernstein.core.agent_lifecycle import _has_git_commits_on_branch, handle_orphaned_task
...
def test_has_git_commits_on_branch_returns_true_when_commits_exist(tmp_path: Path) -> None:
    ...
    with patch("bernstein.core.agent_lifecycle.subprocess") as mock_subprocess:
...
def test_orphaned_task_completes_on_git_commits(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch("bernstein.core.agent_lifecycle._has_git_commits_on_branch", return_value=True),
...
def test_orphaned_task_files_modified_takes_priority(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "bernstein.core.agent_lifecycle.collect_completion_data",
