...
**Was ist das?** Du sagst Bernstein, was gebaut werden soll. Es verteilt die Arbeit auf mehrere KI-Coding-Agenten (Claude Code, Codex, Gemini CLI und 34 weitere), führt die Tests aus und merged genau den Code, der wirklich besteht. Du kommst zurück zu funktionierendem Code.
...
37 CLI-Agent-Adapter: 36 Wrapper für Drittanbieter plus ein generischer Wrapper für alles, was `--prompt` versteht.
...
| Funktioniert mit | Beliebigem CLI-Agenten (37 Adapter) | Python-SDK-Klassen | Python-Agenten | LangChain-Knoten |
...
| Agent-Adapter | 37 | 5 (Kiro, Claude Code, Codex, Gemini, Kimi) | 3 (Claude Code, Codex, Aider) | 24 | 1 (nur Claude Code) |
