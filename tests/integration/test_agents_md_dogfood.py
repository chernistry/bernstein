...
class TestSyncWriteVerifyLoop:
    def test_sync_writes_all_thirteen_canonical_files(self, fixture_repo: Path) -> None:
        ...
        ``N==8`` (7 auto-derived sections + the project-wide ``documentation-duty``
        section produce MDC files), so 13 files total.
        """
        exit_code, output = _invoke(["sync"], fixture_repo)
        assert exit_code == 0, output
        assert "Synced 13 file(s) across 5 target(s)" in output
...
class TestDryRunCounting:
    ...
    def test_sync_dry_run_reports_planned_total(self, fixture_repo: Path) -> None:
        ...
        assert exit_code == 0
        assert "[dry-run] 13 file(s) across 5 target(s) would be synced" in output
