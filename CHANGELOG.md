...
- **Bootstrap composite action for harden-runner + checkout + setup-uv.** Added `.github/actions/bootstrap/action.yml` wrapping `step-security/harden-runner`, `actions/checkout`, and `astral-sh/setup-uv` behind one pinned-SHA call. Inputs cover `fetch-depth`, `python-version`, `enable-uv-cache`, `egress-policy`, `persist-credentials`, and a `setup-uv` toggle for jobs that do not need Python tooling. Migrated `ci.yml` first (22 of the 28 hardened jobs - the install-smoke matrix and aggregator-only jobs stay on direct pins because their patterns do not match the composite shape); remaining workflows migrated in follow-up batches. Net effect: pinned-SHA bumps for the three actions now happen in one file instead of dozens of jobs.
