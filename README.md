...
| `bernstein sandbox web-test <task-id> --url <url> --scenarios <yaml>` | Drives a Playwright self-test against the dev server. Scenario YAML supports `navigate / click / type / assert_visible / screenshot`. Captures screenshots, console messages, and network errors under `.sdd/sandbox/<task-id>/`; optional `--judge` runs the LLM judge. See [docs/sandbox/playwright-self-test.md](docs/sandbox/playwright-self-test.md). |
...
| `bernstein simulate <plan.yaml>` | Digital-twin dry-run: predicts cost band (p50/p90), wall-clock, abandonment probability, per-task blast-radius, and bottlenecks against historical `.sdd/traces/` + `.sdd/metrics/` without spawning a real agent or hitting the network. Supports `--from-traces N`, `--seed S`, `--budget-cap X`, `--out report.json\|report.md`. Exits non-zero when a `--budget-cap` is breached. See [docs/operations/simulate.md](docs/operations/simulate.md). |
| `bernstein compare <spec> --adapters claude,codex[,...]` | Side-by-side adapter A/B in isolated per-adapter worktrees. Up to four adapters, deterministic seed, unified diff against baseline, JSON sidecar at `.sdd/traces/compare-<id>.json`, Markdown summary on stdout. See [docs/operations/compare.md](docs/operations/compare.md). |
