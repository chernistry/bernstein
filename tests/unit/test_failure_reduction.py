...
class TestFileContextDiscovery:
    """TaskContextBuilder discovers relevant files from task text."""

    def test_discovers_files_from_keywords(self, tmp_path: Path) -> None:
        """Keywords in task title/description match against git-tracked files."""
        builder = TaskContextBuilder(tmp_path)
        task = _make_task(
            title="Fix orchestrator spawner bug",
            description="The spawner in orchestrator.py crashes.",
        )
        ...
        assert any("orchestrator" in f for f in result)
        assert any("spawner" in f for f in result)

    def test_returns_empty_when_no_keywords_match(self, tmp_path: Path) -> None:
        """No files returned when task text has no matching keywords."""
        builder = TaskContextBuilder(tmp_path)
        task = _make_task(
            title="Do the thing",
    ...
    def test_build_context_uses_discovery_when_no_owned_files(
        ...
        builder = TaskContextBuilder(tmp_path)
        task = _make_task(
            title="Fix router logic",
            description="The router in router.py has a bug.",
        )
        ...
        assert "Context (auto-generated)" in result
