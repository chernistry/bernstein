...
class ApprovalGate:
    ...
    def create_pr(
        ...
        *,
        ...
        pr_result: PullRequestResult = create_fn(
            cwd=self._workdir,
            title=task.title,
            body=self._pr_body(task, role=role, model=model, cost_usd=cost_usd, test_summary=test_summary),
            head=pr_branch,
            base=base_branch,
    ...
    def _pr_body(
        ...
        *,
        ...
        lines = [f"## {task.title}", "", task.description or "", ""]
        meta: list[str] = []
        if role:
        ...
        if cost_usd > 0:
            meta.append(f"**Cost**: ${cost_usd:.4f}")
        if test_summary:
            meta.append(f"**Tests**: {test_summary}")
        if meta:
            lines.extend(meta)
            lines.append("")
        lines.append(f"---\n*Generated by Bernstein — task `{task.id}`*")
        return "\n".join(lines)
