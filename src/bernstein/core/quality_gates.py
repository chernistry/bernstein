...
class QualityGatesConfig:
    ...
    pii_scan: bool = True
    pii_scan_paths: list[str] = field(default_factory=lambda: ["src/"])  # type: ignore[assignment]
...
def run_quality_gates(
    ...
    if config.pii_scan:
        pii_check = _run_pii_gate(config, run_dir)
        results.append(pii_check)
        _record_gate_event(task.id, "pii_scan", _result_str(pii_check), workdir)
        if not pii_check.passed:
            logger.warning(
                "Quality gate [pii_scan] failed for task %s: %s",
                task.id,
                pii_check.detail[:200],
            )
