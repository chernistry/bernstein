...
class TestHandleOrphanedTaskContextOverflow:
    ...
    def test_context_overflow_triggers_compact_and_retry(self, tmp_path: Path) -> None:
        ...
        with (
            patch("bernstein.core.agent_lifecycle._try_compact_and_retry", return_value=True) as mock_compact,
            patch("bernstein.core.agent_lifecycle.emit_orphan_metrics"),
        ):
    ...
    def test_context_overflow_compact_failure_still_emits_metrics(self, tmp_path: Path) -> None:
        ...
        with (
            patch("bernstein.core.agent_lifecycle._try_compact_and_retry", return_value=False),
            patch("bernstein.core.agent_lifecycle.emit_orphan_metrics") as mock_metrics,
        ):
            handle_orphaned_task(orch, task.id, session, _snapshot(task))
