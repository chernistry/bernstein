...
def _agents_snapshot(request: Request) -> list[dict[str, Any]]:
    ...
    for raw_agent in payload.get("agents", []):
        if isinstance(raw_agent, dict):
            agents_raw.append(
                {
                    "id": str(raw_agent.get("id", "")),
                    "role": str(raw_agent.get("role", "")),
                    "status": str(raw_agent.get("status", "")),
                    "task_id": str(raw_agent.get("task_id", "")),
                    "started_at": raw_agent.get("started_at", ""),
                }
            )
    return agents_raw
...
async def export_tasks(request: Request, format: str = "json") -> Response:
...
async def export_agents(request: Request, format: str = "json") -> Response:
