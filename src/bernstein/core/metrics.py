...
API usage patterns, error rates, and cost efficiency. Stores metrics in
.sdd/metrics/ directory as JSONL files for efficient time-series querying.
...
class MetricsCollector:
    ...
    def complete_task(
        ...
        self._write_metric_point(
            ...
            duration,
            {
                ...
                "success": str(success),
                "janitor_passed": str(janitor_passed),
            },
        )
    ...
    def end_agent(self, agent_id: str) -> AgentMetrics | None:
        ...
        # Record cost efficiency
        if metrics.total_cost_usd > 0 and metrics.tasks_completed > 0:
            cost_per_task = metrics.total_cost_usd / metrics.tasks_completed
            self._write_metric_point(
                MetricType.COST_EFFICIENCY,
                cost_per_task,
                {
    ...
    def record_free_tier_usage(
        ...
        # Record request quota usage
        if requests_limit > 0:
            self._write_metric_point(
                MetricType.FREE_TIER_USAGE,
                requests_used / requests_limit * 100,  # Percentage
                {**labels, "quota_type": "requests"},
            )
        ...
        if tokens_limit > 0:
            self._write_metric_point(
                MetricType.FREE_TIER_USAGE,
                tokens_used / tokens_limit * 100,  # Percentage
    ...
    def record_error(
        ...
        Args:
            ...
            role: Agent role.
        """
        labels = {"error_type": error_type, "provider": provider}
        if model:
            labels["model"] = model
        if role:
            labels["role"] = role
    ...
    def export_metrics(self, output_path: Path) -> None:
        ...
        data = {
            ...
            "task_metrics": [
                ...
                for t in self._task_metrics.values()
            ],
            "agent_metrics": [
                {
                    "agent_id": a.agent_id,
                ...
                for a in self._agent_metrics.values()
            ],
        }
