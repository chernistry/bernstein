...
class ApprovalGate:
    ...
    def create_pr(
        ...
        *,
        ...
        Args:
            ...
            session_id: Agent session ID (the branch is ``agent/{session_id}``).
        ...
        """
        from bernstein.core.git_ops import PullRequestResult, create_github_pr, enable_pr_auto_merge, push_branch
        ...
        push_result = push_fn(worktree_path, agent_branch)
        ...
        # Rename push: use bernstein/task-{id} as head ref for the PR
    ...
    def _pr_body(self, task: Task) -> str:
        return (
            f"## {task.title}\n\n"
            f"{task.description or ''}\n\n"
            "---\n"
            f"*Generated by Bernstein — task `{task.id}`*\n"
        )
