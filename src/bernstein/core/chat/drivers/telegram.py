"""Telegram driver -- python-telegram-bot v22 long-poll.

Standard Telegram bot integration: get a bot API token from
``@BotFather``, set ``BERNSTEIN_TG_TOKEN`` (or pass it via the chat
sink config), and the bridge connects via long-poll. No external
services. ``pip install 'bernstein[telegram]'`` pulls in the
``python-telegram-bot`` library.

The library import is guarded so the module can always be imported --
``python-telegram-bot`` is only required when :meth:`TelegramBridge.start`
actually runs. That keeps ``bernstein chat serve --platform=discord``
working for users who only installed the Discord extra.
