...
| Team adoption dashboard (road-007) | `WORKFLOW-team-adoption-dashboard.md` | Review | v0.2 — Aggregate org-level usage: runs, tasks, cost, quality, merges. 3 Critical gaps verified: cost key mismatch (line 68 reads `total_cost_usd`, writer writes `spent_usd`), quality reads `.json` not `.jsonl`, missing merge writer. 8 RC findings all verified. |
| Multi-modal agent support (road-184) | `WORKFLOW-multi-modal-agent-support.md` | Review | v0.2 — End-to-end attachment pipeline: plan YAML / API → Task → SpawnPrompt → Adapter → Agent. No multi-modal path exists — `Task`, `CLIAdapter.spawn()`, `_render_prompt()` all text-only. 7 RC findings all verified. `ProviderConfig.supports_vision` exists but unwired. |
